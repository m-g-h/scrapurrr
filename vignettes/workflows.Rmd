...
This vignette explains webscraping workflows using `map_scrape()`:
...
3. Apply the `scrapefun()` to the list using `map_scrape()`
...
The following function captures the complete webscraping logic: downloading the page `html` and extracting the relevant content. For use in the `map_scrape()` function provided in this package one thing is important: that it returns a *named list*. This allows to present the output as a dataframe.
...
For this purpose weu use the `map_scrape()` function. It is basically a wrapper around `pmap_dfr()` from `purrr` that provides some webscraping-specific utilities. Let's use it for our search of capitals:
...
This processes can also be implemented using `map_scrape`:
...
# Extraction
results = map_scrape(state_files,
                     .f = get_capitals)
...
Parallelisation is implemented via `furrr`. To scrape in parallel, simply provide the arguments `parallel_workers` and `parallel_strategy` to `map_scrape()`
