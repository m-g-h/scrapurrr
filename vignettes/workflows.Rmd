...
3. Apply the `scrapefun()` to the list using `map_scrape_dfr()`
...
## The `map_scrape...` family of functions

The webscraping workflow in thsi package build upon the `purrr` package and therefore provides its utilities in similarly named functions:

- `map_scrape()` For returning lists
- `map_scrape_dfr()` For returning a dataframe merged rowwise

There are also versions using two inputs which are prefixed with `map2` instead of `map`, as well as parallelised versions using the `furrr` package which are prefixed with `future_`.
...
results = map2_scrape_dfr(state_links, id,
                          download_html)
...
# Extraction
results = map_scrape_dfr(state_files,
                         get_capitals)
